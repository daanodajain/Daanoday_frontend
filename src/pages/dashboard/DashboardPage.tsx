...
import { Users, UserCheck, Receipt, FileBarChart, CreditCard, TrendingUp, AlertCircle, Clock } from 'lucide-react';
...
export const DashboardPage: React.FC = () => {
  ...
  const statsCards = [
    { title: 'Today Collection', value: `₹${Number(stats?.today?.collection || 0).toLocaleString()}`, icon: TrendingUp, color: 'text-emerald-600', bg: 'bg-emerald-100' },
    { title: 'Month Collection', value: `₹${Number(stats?.month?.collection || 0).toLocaleString()}`, icon: CreditCard, color: 'text-blue-600', bg: 'bg-blue-100' },
    { title: 'Total Receipts', value: stats?.total?.receipts || 0, icon: Receipt, color: 'text-purple-600', bg: 'bg-purple-100' },
    { title: t('navigation.customers'), value: stats?.totalCustomers || 0, icon: UserCheck, color: 'text-green-600', bg: 'bg-green-100' },
    ...
    { title: 'Total Customers', value: stats?.totalCustomers || 0, icon: Users, color: 'text-teal-600', bg: 'bg-teal-100' },
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-primary-600 to-primary-700 rounded-lg p-6 text-white">
        <h1 className="text-2xl font-bold mb-1">Welcome to {currentStore?.name || 'Daanoday'}</h1>
            ...
            {recentReceipts.length > 0 ? recentReceipts.map((r: any) => (
              <div key={r.id} className="flex items-center justify-between py-2 border-b border-secondary-100 last:border-0">
                ...
                <div className="text-right">
                  <p className="font-medium">₹{Number(r.total_amount).toLocaleString()}</p>
              ...
              {pendingReceipts.length > 0 ? pendingReceipts.map((r: any) => (
                <div key={r.id} className="flex items-center justify-between py-2 border-b border-secondary-100 last:border-0">
                  <div className="flex-1">
                    <p className="font-medium text-secondary-900">{r.receipt_number}</p>
                    <p className="text-sm text-secondary-600">{r.customer_name} — ₹{Number(r.total_amount).toLocaleString()}</p>
                  </div>
                  <div className="flex gap-2">
                    <Button size="sm" onClick={() => approveMutation.mutate(r.id)} disabled={approveMutation.isPending}>Approve</Button>
                    <Button size="sm" variant="outline" onClick={() => rejectMutation.mutate(r.id)} disabled={rejectMutation.isPending}>Reject</Button>
                  </div>
                </div>
